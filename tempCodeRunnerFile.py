...
def extract_illness_from_response(text):
    ...
    match = re.search(r"\*\*1\. Possible Illnesses or Conditions:\*\*(.*?)\*\*2\.", text, re.DOTALL)
    if match:
...
def scan():
    ...
    try:
        ...
        prompt = (
            ...
            "3. Suggest affordable generic alternatives if possible."
        )
        ...
        ayushman_prompt = f"Is '{detected_illness}' covered under the Ayushman Bharat Yojana scheme? Answer in Yes or No only."
        ...
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(""" 
                INSERT INTO scans (user_id, filename, text, flagged, illness)
                VALUES (?, ?, ?, ?, ?)
